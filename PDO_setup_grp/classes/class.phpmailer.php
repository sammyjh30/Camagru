...
class PHPMailer {
	...
	//(string)An iCal message part body - Only supported in simple alt or alt_inline message types
	//To generate iCal events, use the bundled extras/EasyPeasyICS.php class or iCalcreator (http://sprain.ch/blog/downloads/php-class-easypeasyics-create-ical-files-with-php/) (http://kigkonsult.se/iCalcreator/)
	public $Ical              = '';

	// PROPERTIES, PROTECTED
	
	//(string)Stores the complete compiled MIME message body
	protected $MIMEBody       = '';

	//(string)Stores the complete compiled MIME message headers.
	protected $MIMEHeader     = '';

	//(string)Stores the extra header list which CreateHeader() doesn't fold in
	protected $mailHeader     = '';

	// PROPERTIES, PUBLIC

	//(int)Sets word wrapping on the body of the message to a given number of characters.
	public $WordWrap          = 0;

	//(string)Method to send mail: ("mail", "sendmail", or "smtp").
	public $Mailer            = 'mail';

	//(string)Sets the path of the sendmail program.
	public $Sendmail          = '/usr/sbin/sendmail';

	//(bool)Determine if mail() uses a fully sendmail compatible MTA that supports sendmail's "-oi -f" options
	public $UseSendmailOptions	= true;

	//(string)Path to PHPMailer plugins.  Useful if the SMTP class is in a different directory than the PHP include path.
  	public $PluginDir         = '';

	//(string)Sets the email address that a reading confirmation will be sent.
	public $ConfirmReadingTo  = '';

	//(string)Sets the hostname to use in Message-Id and Received headers and as default HELO string. If empty, the value returned by SERVER_NAME is used or 'localhost.localdomain'.
	public $Hostname          = '';

	//(string)Sets the message ID to be used in the Message-Id header. If empty, a unique id will be generated.
	public $MessageID         = '';

	//(string)Sets the message Date to be used in the Date header. If empty, the current date will be added.
	public $MessageDate       = '';

	// PROPERTIES FOR SMTP

	//(string)Sets the SMTP hosts.
	//All hosts must be separated by a semicolon.  You can also specify a different port for each host by using this format: [hostname:port] (e.g. "smtp1.example.com:25;smtp2.example.com").
	//Hosts will be tried in order.
	public $Host          = 'localhost';

	//(int)Sets the default SMTP server port.
	public $Port          = 25;

	//(string)Sets the SMTP HELO of the message (Default is $Hostname).
	public $Helo          = '';

	//(string)Sets connection prefix. Options are "", "ssl" or "tls"
	public $SMTPSecure    = '';

	//(bool)Sets SMTP authentication. Utilizes the Username and Password variables.
	public $SMTPAuth      = false;

	//(string)Sets SMTP username.
	public $Username      = '';

	//(string)Sets SMTP password.
	public $Password      = '';

	//(string)Sets SMTP auth type. Options are LOGIN | PLAIN | NTLM | CRAM-MD5 (default LOGIN)
  	public $AuthType      = '';

	//(string)Sets SMTP realm.
	public $Realm         = '';

	//(string)Sets SMTP workstation.
	public $Workstation   = '';

	//(int)Sets the SMTP server timeout in seconds. This function will not work with the win32 version.
	public $Timeout       = 10;

	//(bool)Sets SMTP class debugging on or off.
	public $SMTPDebug     = false;

	//(string)Sets the function/method to use for debugging output. Right now we only honor "echo" or "error_log"
	public $Debugoutput     = "echo";

	//(bool)Prevents the SMTP connection from being closed after each mail sending. If this is set to true then to close the connection requires an explicit call to SmtpClose().
	public $SMTPKeepAlive = false;

	//(bool)Provides the ability to have the TO field process individual emails, instead of sending to entire TO addresses
	public $SingleTo      = false;

	//(bool)Should we generate VERP addresses when sending via SMTP? (http://en.wikipedia.org/wiki/Variable_envelope_return_path)
	public $do_verp      = false;

	//(bool)If SingleTo is true, this provides the array to hold the email addresses
	public $SingleToArray = array();

	//(bool)Should we allow sending messages with empty body?
	public $AllowEmpty = false;

	//(string)Provides the ability to change the generic line ending
	//NOTE: The default remains '\n'. We force CRLF where we KNOW it must be used via self::CRLF
	public $LE              = "\n";
	
	//(string)Used with DKIM Signing - required parameter if DKIM is enabled - domain selector example domainkey
	public $DKIM_selector   = '';

	//(string)Used with DKIM Signing - required if DKIM is enabled, in format of email address 'you@yourdomain.com' typically used as the source of the email
	public $DKIM_identity   = '';

	//(string)Used with DKIM Signing - optional parameter if your private key requires a passphras
	public $DKIM_passphrase   = '';

	//Used with DKIM Singing - required if DKIM is enabled, in format of email address 'domain.com'
	public $DKIM_domain     = '';

	//(string)Used with DKIM Signing - required if DKIM is enabled, path to private key file
	public $DKIM_private    = '';
  ...
  public $action_function = ''; //'callbackAction';
	...
	//(string)Sets the PHPMailer Version number
	public $Version         = '5.2.6';

	//(string)What to use in the X-Mailer header - NULL for default, whitespace for None, or actual string to use
	public $XMailer         = '';
	...
	//(int)The number of errors encountered
	...
	//(bool)Whether to throw exceptions for errors
	...
	//(string)Outputs debugging info via user-defined method
	//@param string $str
